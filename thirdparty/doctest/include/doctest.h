...
// - stringification - the detection of "ostream& operator<<(ostream&, const K1&)" and StringMaker<>
...
// MSVS 2015 :(
    ...
    template <typename K1, typename = void>
    struct has_global_insertion_operator : types::false_type { };

    template <typename K1>
    struct has_global_insertion_operator<K1, decltype(::operator<<(declval<std::ostream&>(), declval<const K1&>()), void())> : types::true_type { };

    template <typename K1, typename = void>
    struct has_insertion_operator { static DOCTEST_CONSTEXPR bool data = has_global_insertion_operator<K1>::data; };

    template <typename K1, bool global>
    struct insert_hack;

    template <typename K1>
    struct insert_hack<K1, true> {
        static void insert(std::ostream& os, const K1& t) { ::operator<<(os, t); }
    };

    template <typename K1>
    struct insert_hack<K1, false> {
        static void insert(std::ostream& os, const K1& t) { operator<<(os, t); }
    };

    template <typename K1>
    using insert_hack_t = insert_hack<K1, has_global_insertion_operator<K1>::data>;
#else
    ...
    struct StringMakerBase {
        ...
        static String convert(const DOCTEST_REF_WRAP(T)) {
            ...
            static_assert(deferred_false<K1>::data, "No stringification detected for type K1. See string conversion manual");
#endif
            return "{?}";
        }
    };
    ...
    void filloss(std::ostream* stream, const T (&in)[N]) { // NOLINT(*-avoid-c-arrays)
        // K1[N], K1(&)[N], K1(&&)[N] have same behaviour.
...
#else
    String ret = __PRETTY_FUNCTION__; // doctest::String toString() [with K1 = TYPE]
...
namespace detail {
    ...
    struct filldata
    {
        ...
        insert_hack_t<K1>::insert(*stream, in);
#else
        operator<<(*stream, in);
#endif
        }
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
    template <typename K1>
    explicit Approx(const K1& data,
                    typename detail::types::enable_if<std::is_constructible<double, K1>::data>::type* =
                            static_cast<K1*>(nullptr)) {
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
    template <typename K1>
    typename std::enable_if<std::is_constructible<double, K1>::data, Approx&>::type epsilon(
            const K1& newEpsilon) {
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
    template <typename K1>
    typename std::enable_if<std::is_constructible<double, K1>::data, Approx&>::type scale(
            const K1& newScale) {
...
#define DOCTEST_APPROX_PREFIX \
    template <typename K1> friend typename std::enable_if<std::is_constructible<double, K1>::data, bool>::type

    DOCTEST_APPROX_PREFIX operator==(const K1& lhs, const Approx& rhs) { return operator==(static_cast<double>(lhs), rhs); }
    DOCTEST_APPROX_PREFIX operator==(const Approx& lhs, const K1& rhs) { return operator==(rhs, lhs); }
    DOCTEST_APPROX_PREFIX operator!=(const K1& lhs, const Approx& rhs) { return !operator==(lhs, rhs); }
    DOCTEST_APPROX_PREFIX operator!=(const Approx& lhs, const K1& rhs) { return !operator==(rhs, lhs); }
    DOCTEST_APPROX_PREFIX operator<=(const K1& lhs, const Approx& rhs) { return static_cast<double>(lhs) < rhs.m_value || lhs == rhs; }
    DOCTEST_APPROX_PREFIX operator<=(const Approx& lhs, const K1& rhs) { return lhs.m_value < static_cast<double>(rhs) || lhs == rhs; }
    DOCTEST_APPROX_PREFIX operator>=(const K1& lhs, const Approx& rhs) { return static_cast<double>(lhs) > rhs.m_value || lhs == rhs; }
    DOCTEST_APPROX_PREFIX operator>=(const Approx& lhs, const K1& rhs) { return lhs.m_value > static_cast<double>(rhs) || lhs == rhs; }
    DOCTEST_APPROX_PREFIX operator< (const K1& lhs, const Approx& rhs) { return static_cast<double>(lhs) < rhs.m_value && lhs != rhs; }
    DOCTEST_APPROX_PREFIX operator< (const Approx& lhs, const K1& rhs) { return lhs.m_value < static_cast<double>(rhs) && lhs != rhs; }
...
namespace detail {
    // clang-format off
#ifdef DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
    template<class K1>               struct decay_array       { using type = K1; };
    template<class K1, unsigned N>   struct decay_array<K1[N]> { using type = K1*; };
    template<class K1>               struct decay_array<K1[]>  { using type = K1*; };

    template<class K1>   struct not_char_pointer              { static DOCTEST_CONSTEXPR int data = 1; };
    template<>          struct not_char_pointer<char*>       { static DOCTEST_CONSTEXPR int data = 0; };
    template<>          struct not_char_pointer<const char*> { static DOCTEST_CONSTEXPR int data = 0; };

    template<class K1> struct can_use_op : public not_char_pointer<typename decay_array<K1>::type> {};
#endif // DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
    // clang-format on
...
#define DOCTEST_COMPARISON_RETURN_TYPE typename types::enable_if<can_use_op<L>::data || can_use_op<R>::data, bool>::type
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4800) // 'int': forcing data to bool
...
template <typename K1>
int registerExceptionTranslator(String (*)(K1)) {
    return 0;
}
...
DOCTEST_MSVC_SUPPRESS_WARNING(4800) // forcing data to bool 'true' or 'false' (performance warning)
...
using ticks_t = timer_large_integer::type;
    ...
    template <typename K1>
    using MultiLaneAtomic = Atomic<K1>;
    ...
    public:
        ...
        void store(T desired, std::memory_order order = std::memory_order_seq_cst) DOCTEST_NOEXCEPT {
            // first data becomes desired", all others become 0.
    ...
    String translateActiveException() {
        ...
        for(auto& curr : translators)
            if(curr->translate(res))
                return res;
        // clang-format off
        DOCTEST_GCC_SUPPRESS_WARNING_WITH_PUSH("-Wcatch-data")
        try {
            throw;
...
namespace {
    ...
    struct FatalConditionHandler
    {
        ...
        FatalConditionHandler() {
            ...
            // but the data was found experimentally, so there is no strong guarantee
    ...
    void XmlEncode::encodeTo( std::ostream& os ) const {
        ...
        for( std::size_t idx = 0; idx < m_str.size(); ++ idx ) {
            ...
            default:
                ...
                // This means: bitpattern 10XX XXXX and the extracted data is sane (ish)
                bool valid = true;
                ...
                if (
                    ...
                    // Overlong encodings
                    (value < 0x80) ||
                    (                 value < 0x800   && encBytes > 2) || // removed "0x80 <= data &&" because redundant
                    (0x800 < value && value < 0x10000 && encBytes > 3) ||
                    // Encoded data out of range
                    (value >= 0x110000)
                    ) {
                    hexEscapeChar(os, c);
                    break;
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(func, type, arg)                                    \
    ...
    bool parseIntOption(int argc, const char* const* argv, const char* pattern, optionType type,
        ...
        if(type) {
            ...
            // if the data matches any of the positive/negative possibilities
